...
export default function RegisterPage() {
  ...
  const submit = async (e: FormEvent) => {
    ...
    try {
      ...
      setSuccess(result?.message || tr('Compte cree. Confirmez votre email pour vous connecter.', 'Voasoratra ny kaonty. Hamarino ny email alohan\'ny hidirana.'));
      router.push(`/login?registered=1&email=${encodeURIComponent(email.trim())}`);
    } catch (e: any) {
      setError(e.message || tr("Echec de l'inscription.", 'Tsy tafavoaka ny fisoratana anarana.'));
    }
  };
      ...
      <section className="card cardBody authFormCard">
        ...
        <form className="grid" style={{ gap: '0.85rem' }} onSubmit={submit}>
          ...
          <div className="formGrid">
            ...
            <div className="formField">
              <label className="formLabel" htmlFor="register-phone">{tr('Telephone', 'Telefaona')}</label>
          ...
          <button className="primaryBtn" type="submit">{tr('Creer mon compte', 'Mamorona kaontiko')}</button>
        </form>
        ...
        <p className="muted" style={{ margin: 0 }}>
          {tr('Deja inscrit ?', 'Efa nisoratra anarana?')}{' '}
